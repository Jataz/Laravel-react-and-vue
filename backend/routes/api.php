...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::middleware(['role:admin,super-admin'])->group(function () {
        // User management
        Route::get('/users', [UserController::class, 'index'])->middleware(['permission:view users', 'redis.cache:300']);
        Route::get('/users/{user}', [UserController::class, 'show'])->middleware(['permission:view users', 'redis.cache:600']);
        ...
        Route::get('/users/{user}/permissions', [UserController::class, 'getUserPermissions'])->middleware(['permission:view users', 'redis.cache:300']);

        // Role management
        Route::get('/roles', [RoleController::class, 'index'])->middleware(['permission:view roles', 'redis.cache:600']);
        Route::post('/roles', [RoleController::class, 'store'])->middleware('permission:create roles');
        Route::get('/roles/{role}', [RoleController::class, 'show'])->middleware(['permission:view roles', 'redis.cache:600']);
        ...
        Route::get('/permissions', [PermissionController::class, 'index'])->middleware(['permission:view permissions', 'redis.cache:600']);
        Route::post('/permissions', [PermissionController::class, 'store'])->middleware('permission:create permissions');
        Route::get('/permissions/{permission}', [PermissionController::class, 'show'])->middleware(['permission:view permissions', 'redis.cache:600']);
...
// Redis testing routes (public for testing purposes)
Route::prefix('redis')->group(function () {
    Route::get('test-cache', [RedisTestController::class, 'testCache']);
    Route::get('test-connection', [RedisTestController::class, 'testConnection']);
    Route::get('server-info', [RedisTestController::class, 'serverInfo']);
    Route::get('test-api-caching', [RedisTestController::class, 'testApiCaching']);
